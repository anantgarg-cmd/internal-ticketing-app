...
function filterDate_(value,label){value=String(value||'').trim();if(!value)return null;if(!/^\d{4}-\d{2}-\d{2}$/.test(value))throw new Error('Invalid '+label+'.');const date=new Date(value+'T00:00:00Z');if(isNaN(date.getTime()))throw new Error('Invalid '+label+'.');return date;}
...
function getAllTickets(options) {
  ...
  const status=filterValue_(options.status,TICKET_FILTER_VALUES_.status,'status'),priority=filterValue_(options.priority,TICKET_FILTER_VALUES_.priority,'priority'),sla=filterValue_(options.sla,TICKET_FILTER_VALUES_.sla,'SLA'),raisedBy=exactRegisteredEmail_(options.raisedBy,users,user),from=filterDate_(options.createdFrom,'created date'),to=filterDate_(options.createdTo,'created date');validateRange_(from,to);if(to)to=new Date(to.getTime()+86400000-1);
  const all=getSheetObjects_(APP.SHEETS.TICKET_INDEX),visible=all.filter(t=>ticketVisibleTo_(t,user,roleMap,cutoff));
  const filtered=visible.filter(t=>ticketMatchesActivitySearch_(t,search)).filter(t=>(!raisedBy||lower_(t.Raiser_Email)===raisedBy)&&(!status||String(t.Status)===status)&&(!priority||String(t.Priority)===priority)&&(!sla||ticketSlaFilter_(t)===sla)&&(!options.clientType||String(t.Client_Type)===String(options.clientType).trim())&&(!options.clientSize||String(t.Client_Size)===String(options.clientSize).trim())&&(!options.category||String(t.Category_Group||t.Category_Name)===String(options.category).trim())&&(!options.subcategory||String(t.Subcategory_Name||t.Category_Name)===String(options.subcategory).trim())&&(!options.client||[t.Client_ID,t.Client_Name].some(v=>lower_(v).includes(lower_(options.client).trim())))&&(!options.resolvedBy||lower_(t.Resolved_By)===lower_(options.resolvedBy).trim())&&(!from||toDate_(t.Created_At)>=from)&&(!to||toDate_(t.Created_At)<=to)).sort((a,b)=>toDate_(b.Created_At)-toDate_(a.Created_At));
...
function getQueueTickets(filters) {
  filters=filters||{};
  const startedAt=Date.now(),user=requireRole_([APP.ROLES.POC,APP.ROLES.ADMIN]);const users=getSheetObjects_(APP.SHEETS.USERS),search=lower_(filters.search).trim(),status=filters.status===undefined?'OPEN':String(filters.status).trim();if(status!=='OPEN')filterValue_(status,TICKET_FILTER_VALUES_.status,'status');const priority=filterValue_(filters.priority,TICKET_FILTER_VALUES_.priority,'priority'),sla=filterValue_(filters.sla,TICKET_FILTER_VALUES_.sla,'SLA'),raisedBy=exactRegisteredEmail_(filters.raisedBy,users,user),picked=lower_(filters.pickedUpBy).trim()==='me'?user.email:lower_(filters.pickedUpBy).trim();
  const all=getSheetObjects_(APP.SHEETS.TICKET_INDEX),filtered=all.filter(t=>{const x=serializeQueueTicket_(t);return(!search||ticketMatchesActivitySearch_(t,search))&&(status==='OPEN'?['Raised','Reopened','Investigating'].includes(x.status):(!status||x.status===status))&&(!priority||x.priority===priority)&&(!filters.category||String(t.Category_Group||t.Category_Name)===String(filters.category).trim())&&(!filters.subcategory||String(t.Subcategory_Name||t.Category_Name)===String(filters.subcategory).trim())&&(!filters.clientType||x.clientType===String(filters.clientType).trim())&&(!filters.clientSize||x.clientSize===String(filters.clientSize).trim())&&(!sla||ticketSlaFilter_(t)===sla)&&(!raisedBy||lower_(t.Raiser_Email)===raisedBy)&&(!filters.unassignedOnly||!lower_(t.Picked_Up_By))&&(!picked||(picked==='unassigned'?!lower_(t.Picked_Up_By):lower_(t.Picked_Up_By)===picked));}).sort(queueSort_);
