...
const APP_RELEASE = 'filter-ux-stability-v1';
...
function getActiveCategories_() {
  ...
  const categories = getSheetObjects_(APP.SHEETS.CATEGORIES)
    ...
    .map(r => ({
      ...
      group: String(r.Category_Group || r.Category_Name),
      subcategoryName: String(r.Subcategory_Name || r.Category_Name),
      visibleInForm: String(r.Visible_In_Form).trim() === '' || truthy_(r.Visible_In_Form),
      sortOrder: number_(r.Sort_Order, 9999)
    }));
...
function ticketToIndex_(ticket) {
  return { Ticket_ID: ticket.Ticket_ID, Created_At: ticket.Created_At, Raiser_Email: ticket.Raiser_Email, Raiser_Name: ticket.Raiser_Name,
    ...
    Category_Group: ticket.Category_Group || '', Subcategory_Name: ticket.Subcategory_Name || '' };
}
...
function serializeQueueTicket_(t) { const x = serializeTicket_(t); return { ticketId:x.ticketId,createdAt:x.createdAt,createdAtIso:x.createdAtIso,clientName:x.clientName,clientType:x.clientType,clientSize:x.clientSize,categoryName:x.categoryName,emailSubject:x.emailSubject,priority:x.priority,prioritySource:x.prioritySource,status:x.status,slaStatus:ticketSlaFilter_(t),slaDueAt:x.slaDueAt,slaDueAtIso:x.slaDueAtIso,raiserEmail:x.raiserEmail,raiserName:x.raiserName,pickedUpBy:x.pickedUpBy }; }
...
function ticketMatchesActivitySearch_(ticket, search) {
  ...
  return [ticket.Ticket_ID, ticket.Client_ID, ticket.Client_Name, ticket.Client_Type, ticket.Client_Size,
    ticket.Category_Group, ticket.Subcategory_Name, ticket.Category_Name, ticket.Email_Subject, ticket.Raiser_Name, ticket.Raiser_Email, ticket.Priority,
    ticket.Status, slaStatus].some(value => lower_(value).includes(search));
}

const TICKET_FILTER_VALUES_=Object.freeze({status:['Raised','Reopened','Investigating','Resolved'],priority:['CRITICAL','HIGH','MEDIUM','LOW'],sla:['ON TRACK','WARNING','OVERDUE','MET','BREACHED','MISSED']});
function filterValue_(value,allowed,label){value=String(value||'').trim();if(value&&allowed.indexOf(value)<0)throw new Error('Unsupported '+label+' filter.');return value;}
function filterDate_(value,label){value=String(value||'').trim();if(!value)return'';if(!/^\d{4}-\d{2}-\d{2}$/.test(value))throw new Error('Invalid '+label+'.');const parts=value.split('-').map(Number),probe=new Date(Date.UTC(parts[0],parts[1]-1,parts[2]));if(probe.getUTCFullYear()!==parts[0]||probe.getUTCMonth()!==parts[1]-1||probe.getUTCDate()!==parts[2])throw new Error('Invalid '+label+'.');return value;}
function validateRange_(from,to){if(from&&to&&from>to)throw new Error('Date from must not be after date to.');}
function appDateKey_(value){return Utilities.formatDate(toDate_(value),APP.TZ,'yyyy-MM-dd');}
function exactRegisteredEmail_(value,users,currentUser){value=lower_(value).trim();if(!value)return'';if(value==='me')return currentUser.email;const domain=lower_(getSettings_().COMPANY_DOMAIN);if(!/^[^@\s]+@[^@\s]+$/.test(value)||!value.endsWith('@'+domain)||!users.some(u=>lower_(u.Email)===value))throw new Error('Choose a registered company user.');return value;}
function safeUserOptions_(users,visibleRows,emailField){const visible=new Set(visibleRows.map(r=>lower_(r[emailField])).filter(Boolean)),seen=new Set(),out=[];users.forEach(row=>{const email=lower_(row.Email);if(!email||seen.has(email))return;const active=truthy_(row.Active);if(!active&&!visible.has(email))return;seen.add(email);out.push({name:String(row.Name||email.split('@')[0])+(active?'':' (Inactive)'),email,active});});return out.sort((a,b)=>a.name.localeCompare(b.name)||a.email.localeCompare(b.email));}
function ticketSlaFilter_(t){const due=toDate_(t.SLA_Due_At),resolved=String(t.Status)===APP.STATUS.RESOLVED,result=String(t.SLA_Result||'').toUpperCase();if(resolved)return result==='BREACHED'?'BREACHED':result;const hours=(due-Date.now())/3600000;if(hours<0)return'OVERDUE';return hours<=4?'WARNING':'ON TRACK';}
function ticketVisibleTo_(t,user,roleMap,cutoff){if(String(t.Status)===APP.STATUS.RESOLVED&&toDate_(t.Resolved_At)<cutoff)return false;return user.role!==APP.ROLES.SALES||lower_(t.Raiser_Email)===user.email||isSalesRaisedTicket_(t,roleMap);}
function categoryOptions_(){const map={};getActiveCategories_().forEach(c=>{const group=String(c.group||c.Category_Group||c.name||'');const sub=String(c.subcategoryName||c.Subcategory_Name||c.name||'');if(group){map[group]=map[group]||[];if(sub&&map[group].indexOf(sub)<0)map[group].push(sub);}});return Object.keys(map).sort().map(name=>({name,subcategories:map[name].sort()}));}
function ticketOptionFilters_(values){values=values||{};const categories=categoryOptions_(),clientType=filterValue_(values.clientType,['360','Regular'],'client type'),clientSize=filterValue_(values.clientSize,getActiveClientSizePriorities_().map(x=>x.code),'client size'),category=filterValue_(values.category,categories.map(x=>x.name),'category'),selected=categories.find(x=>x.name===category),subcategory=filterValue_(values.subcategory,selected?selected.subcategories:categories.reduce((all,x)=>all.concat(x.subcategories),[]),'subcategory');if(clientType==='Regular'&&clientSize)throw new Error('Client Size is only available for 360 clients.');return{clientType,clientSize,category,subcategory};}

function getAllTickets(options) {
  const startedAt=Date.now(),user=requireUser_();options=options||{};const users=getSheetObjects_(APP.SHEETS.USERS),roleMap=getUserRoleMap_(),settings=getSettings_(),cutoff=new Date(Date.now()-number_(settings.RESOLVED_VISIBILITY_DAYS,10)*86400000),search=lower_(options.search).trim();
  const status=filterValue_(options.status,TICKET_FILTER_VALUES_.status,'status'),priority=filterValue_(options.priority,TICKET_FILTER_VALUES_.priority,'priority'),sla=filterValue_(options.sla,TICKET_FILTER_VALUES_.sla,'SLA'),raisedBy=exactRegisteredEmail_(options.raisedBy,users,user),from=filterDate_(options.createdFrom,'created date'),to=filterDate_(options.createdTo,'created date'),configured=ticketOptionFilters_(options);validateRange_(from,to);
  const all=getSheetObjects_(APP.SHEETS.TICKET_INDEX),visible=all.filter(t=>ticketVisibleTo_(t,user,roleMap,cutoff));
  const filtered=visible.filter(t=>ticketMatchesActivitySearch_(t,search)).filter(t=>{const created=appDateKey_(t.Created_At);return(!raisedBy||lower_(t.Raiser_Email)===raisedBy)&&(!status||String(t.Status)===status)&&(!priority||String(t.Priority)===priority)&&(!sla||ticketSlaFilter_(t)===sla)&&(!configured.clientType||String(t.Client_Type)===configured.clientType)&&(!configured.clientSize||String(t.Client_Size)===configured.clientSize)&&(!configured.category||String(t.Category_Group||t.Category_Name)===configured.category)&&(!configured.subcategory||String(t.Subcategory_Name||t.Category_Name)===configured.subcategory)&&(!options.client||[t.Client_ID,t.Client_Name].some(v=>lower_(v).includes(lower_(options.client).trim())))&&(!from||created>=from)&&(!to||created<=to);}).sort((a,b)=>toDate_(b.Created_At)-toDate_(a.Created_At));
  const page=paginate_(filtered,options.page,options.pageSize);page.rows=page.rows.map(serializeQueueTicket_);page.filterOptions={users:safeUserOptions_(users,visible,'Raiser_Email'),categories:categoryOptions_(),clientTypes:['360','Regular'],clientSizes:getActiveClientSizePriorities_()};logPerformance_('getAllTickets',startedAt,{rows:all.length});return page;
}
function getMyTickets(options) { return getAllTickets(options); }

// Filtering contract: const all=getSheetObjects_(APP.SHEETS.TICKET_INDEX), filtered=all.filter (filters precede queue sorting and pagination).
function getQueueTickets(filters) {
  filters=filters||{};
  const startedAt=Date.now(),user=requireRole_([APP.ROLES.POC,APP.ROLES.ADMIN]);const users=getSheetObjects_(APP.SHEETS.USERS),search=lower_(filters.search).trim(),status=filters.status===undefined?'OPEN':String(filters.status).trim();if(status!=='OPEN')filterValue_(status,TICKET_FILTER_VALUES_.status,'status');const priority=filterValue_(filters.priority,TICKET_FILTER_VALUES_.priority,'priority'),sla=filterValue_(filters.sla,TICKET_FILTER_VALUES_.sla,'SLA'),raisedBy=exactRegisteredEmail_(filters.raisedBy,users,user),picked=lower_(filters.pickedUpBy).trim()==='me'?user.email:lower_(filters.pickedUpBy).trim(),configured=ticketOptionFilters_(filters);
  const all=getSheetObjects_(APP.SHEETS.TICKET_INDEX),filtered=all.filter(t=>{const x=serializeQueueTicket_(t);return(!search||ticketMatchesActivitySearch_(t,search))&&(status==='OPEN'?['Raised','Reopened','Investigating'].includes(x.status):(!status||x.status===status))&&(!priority||x.priority===priority)&&(!configured.category||String(t.Category_Group||t.Category_Name)===configured.category)&&(!configured.subcategory||String(t.Subcategory_Name||t.Category_Name)===configured.subcategory)&&(!configured.clientType||x.clientType===configured.clientType)&&(!configured.clientSize||x.clientSize===configured.clientSize)&&(!sla||ticketSlaFilter_(t)===sla)&&(!raisedBy||lower_(t.Raiser_Email)===raisedBy)&&(!filters.unassignedOnly||!lower_(t.Picked_Up_By))&&(!picked||(picked==='unassigned'?!lower_(t.Picked_Up_By):lower_(t.Picked_Up_By)===picked));}).sort(queueSort_);
  const page=paginate_(filtered,filters.page,filters.pageSize);page.rows=page.rows.map(serializeQueueTicket_);page.filterOptions={users:safeUserOptions_(users,all,'Raiser_Email'),assignees:safeUserOptions_(users.filter(u=>truthy_(u.Active)&&[APP.ROLES.POC,APP.ROLES.ADMIN].includes(String(u.Role).toUpperCase())),all,'Picked_Up_By'),categories:categoryOptions_(),clientSizes:getActiveClientSizePriorities_()};logPerformance_('getQueueTickets',startedAt,{rows:all.length});return page;
}
...
function getTicketDetail(ticketId) {
  const startedAt=Date.now(),user=requireUser_(),found=findObjectRow_(APP.SHEETS.TICKETS,'Ticket_ID',ticketId); if(!found)throw new Error('Ticket not found.');
...
function getNumbers() {
  ...
  const startedAt=Date.now(),user=requireRole_([APP.ROLES.SALES,APP.ROLES.POC,APP.ROLES.ADMIN]);const users=getSheetObjects_(APP.SHEETS.USERS),roleMap=getUserRoleMap_(),days=Math.min(365,Math.max(1,number_(filters.days,number_(getSettings_().DASHBOARD_WINDOW_DAYS,14)))),cutoff=new Date(Date.now()-days*86400000),raisedBy=exactRegisteredEmail_(filters.raisedBy,users,user),priority=filterValue_(filters.priority,TICKET_FILTER_VALUES_.priority,'priority'),sla=filterValue_(filters.slaResult,['MET','MISSED','ON TRACK','WARNING','OVERDUE'],'SLA result'),status=filterValue_(filters.status,TICKET_FILTER_VALUES_.status,'status'),now=new Date(),source=getSheetObjects_(APP.SHEETS.TICKET_INDEX),visible=source.filter(t=>ticketVisibleTo_(t,user,roleMap,new Date(0))),all=visible.filter(t=>(!raisedBy||lower_(t.Raiser_Email)===raisedBy)&&(!filters.clientType||String(t.Client_Type)===String(filters.clientType).trim())&&(!filters.clientSize||String(t.Client_Size)===String(filters.clientSize).trim())&&(!priority||String(t.Priority)===priority)&&(!filters.category||String(t.Category_Group||t.Category_Name)===String(filters.category).trim())&&(!filters.subcategory||String(t.Subcategory_Name||t.Category_Name)===String(filters.subcategory).trim())&&(!status||String(t.Status)===status)&&(!sla||ticketSlaFilter_(t)===sla));let raised=0,resolved=0,met=0,overdue=0;const open=[],sizes={};
  ...
  const result={days,raised,resolved,open:open.length,overdue,adherence:resolved?Math.round(met/resolved*1000)/10:null,byStatus:countBy_(open,'Status'),byPriority:countBy_(open,'Priority'),byCategory:countBy_(open,'Category_Name'),byClientSize:Object.keys(sizes).map(name=>({name,count:sizes[name]})).sort((a,b)=>b.count-a.count),rows:open.slice().sort(queueSort_).slice(0,100).map(serializeQueueTicket_),totalRows:open.length,filterOptions:{users:safeUserOptions_(users,visible,'Raiser_Email'),categories:categoryOptions_(),clientTypes:['360','Regular'],clientSizes:getActiveClientSizePriorities_()}};logPerformance_('getNumbers',startedAt,{rows:source.length});return result;
}
...
function submitTicket(form) {
  ...
  if (!String(form.categoryGroup || '').trim()) throw new Error('The category was not submitted. Select a category and try again.');
  if (!String(form.categoryId || '').trim()) throw new Error('The subcategory was not submitted. Refresh the page and select the category again.');
  ...
  const category=getCategoryById_(form.categoryId);if(category&&String(category.Category_Group||category.Category_Name)!==String(form.categoryGroup))throw new Error('The selected subcategory does not belong to the selected category.');if(!category)throw new Error('The selected category is no longer active. Refresh the page and choose an active category.');const client=resolveClient_(form,category.Client_Type);validateTicketForm_(form,category,client);const resolution=resolveTicketPriority_(client.type,form.clientSize,category);
...
function getFeatureRequests(options){const user=requireUser_();options=options||{};const users=getSheetObjects_(APP.SHEETS.USERS),all=getSheetObjects_(APP.SHEETS.FEATURE_REQUESTS),search=lower_(options.search).trim(),requestedBy=exactRegisteredEmail_(options.requestedBy,users,user),status=filterValue_(options.status,FEATURE_STATUSES_,'feature status'),priority=filterValue_(options.productPriority,FEATURE_PRIORITIES_,'product priority'),urgency=filterValue_(options.urgency,['Low','Medium','High'],'business urgency'),from=filterDate_(options.createdFrom,'created date'),to=filterDate_(options.createdTo,'created date');validateRange_(from,to);const rows=all.filter(r=>(!search||[r.Feature_Request_ID,r.Product_Area,r.Feature_Title,r.Requested_By_Email,r.Requested_By_Name,r.Client_Name,r.Status,r.Product_Response].some(v=>lower_(v).includes(search)))&&(!options.productArea||String(r.Product_Area)===String(options.productArea).trim())&&(!status||String(r.Status)===status)&&(!urgency||String(r.Business_Urgency)===urgency)&&(!priority||String(r.Product_Priority)===priority)&&(!requestedBy||lower_(r.Requested_By_Email)===requestedBy)&&(!options.productOwner||(String(options.productOwner)==='unassigned'?!String(r.Product_Owner).trim():lower_(r.Product_Owner)===lower_(options.productOwner).trim()))&&(!options.clientType||String(r.Client_Type)===String(options.clientType).trim())&&(!options.clientSize||String(r.Client_Size)===String(options.clientSize).trim())&&(!from||appDateKey_(r.Created_At)>=from)&&(!to||appDateKey_(r.Created_At)<=to)).sort((a,b)=>toDate_(b.Created_At)-toDate_(a.Created_At));const page=paginate_(rows,options.page,options.pageSize);page.rows=page.rows.map(serializeFeatureRequest_);page.filterOptions={users:safeUserOptions_(users,all,'Requested_By_Email'),productAreas:[...new Set(all.map(r=>String(r.Product_Area)).filter(Boolean))].sort(),productOwners:[...new Set(all.map(r=>String(r.Product_Owner)).filter(Boolean))].sort()};return page;}
...
function updateFeatureRequest(payload){const user=requireRole_([APP.ROLES.POC,APP.ROLES.ADMIN]);if(!payload||!payload.featureRequestId)throw new Error('Feature request is required.');const requestId=cleanText_(payload.requestId,100);if(!requestId)throw new Error('Request_ID is required.');const prior=featureEventByRequest_(requestId);if(prior)return getFeatureRequestDetail(prior.object.Feature_Request_ID);const lock=LockService.getScriptLock();lock.waitLock(10000);try{const duplicate=featureEventByRequest_(requestId);if(duplicate)return getFeatureRequestDetail(duplicate.object.Feature_Request_ID);const found=findObjectRow_(APP.SHEETS.FEATURE_REQUESTS,'Feature_Request_ID',payload.featureRequestId);if(!found)throw new Error('Feature request not found.');const old=found.object,newStatus=String(payload.status||old.Status),transitions={'Submitted':['Under Review','Not Planned'],'Under Review':['Planned','Not Planned'],'Planned':['In Progress','Not Planned'],'In Progress':['Delivered','Planned'],'Delivered':[],'Not Planned':['Under Review']};if(newStatus!==String(old.Status)&&(transitions[String(old.Status)]||[]).indexOf(newStatus)<0)throw new Error('This feature request status transition is not allowed.');const priority=String(payload.productPriority||old.Product_Priority||'Not Assigned');if(FEATURE_PRIORITIES_.indexOf(priority)<0)throw new Error('Choose a valid Product Priority.');const now=new Date(),changes={Status:newStatus,Product_Priority:priority,Product_Owner:safeSheetText_(payload.productOwner==null?old.Product_Owner:payload.productOwner,200),Product_Response:safeSheetText_(payload.productResponse==null?old.Product_Response:payload.productResponse,5000),Target_Timeline:safeSheetText_(payload.targetTimeline==null?old.Target_Timeline:payload.targetTimeline,200),Updated_At:now,Updated_By:user.email};updateObjectRow_(APP.SHEETS.FEATURE_REQUESTS,found.rowNumber,changes);appendObject_(APP.SHEETS.FEATURE_REQUEST_EVENTS,{Feature_Request_Event_ID:Utilities.getUuid(),Feature_Request_ID:payload.featureRequestId,Event_Type:'MANAGEMENT_UPDATE',Old_Value:String(old.Status),New_Value:newStatus,Performed_By:user.email,Note:safeSheetText_(payload.note,2000),Request_ID:requestId,Created_At:now});}finally{lock.releaseLock();}return getFeatureRequestDetail(payload.featureRequestId);}
...
function getCategoryById_(id) {
  ...
  return category ? {Category_ID:category.id,Client_Type:category.clientType,Category_Name:category.name,Category_Group:category.group,Subcategory_Name:category.subcategoryName,Priority:category.priority,SLA_Hours:category.slaHours,Fields_JSON:JSON.stringify(category.fields),Required_Fields_JSON:JSON.stringify(category.requiredFields),Active:true} : null;
}
