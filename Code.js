...
function getAllTickets(options) {
  const startedAt=Date.now();requireUser_();options=options||{};const settings=getSettings_(),cutoff=new Date(Date.now()-number_(settings.RESOLVED_VISIBILITY_DAYS,10)*86400000),search=lower_(options.search).trim();
  const all=getSheetObjects_(APP.SHEETS.TICKET_INDEX),filtered=all.filter(t=>String(t.Status)!==APP.STATUS.RESOLVED||toDate_(t.Resolved_At)>=cutoff).filter(t=>ticketMatchesActivitySearch_(t,search)).filter(t=>(!options.status||String(t.Status)===String(options.status))&&(!options.priority||String(t.Priority)===String(options.priority))&&(!options.category||String(t.Category_Group||t.Category_Name)===String(options.category))).sort((a,b)=>toDate_(b.Created_At)-toDate_(a.Created_At));
  const page=paginate_(filtered,options.page,options.pageSize);page.rows=page.rows.map(serializeQueueTicket_);logPerformance_('getAllTickets',startedAt,{rows:all.length});return page;
}
...
function getNumbers() {
  const startedAt=Date.now();requireRole_([APP.ROLES.SALES,APP.ROLES.POC,APP.ROLES.ADMIN]);const cached=getCachedJson_(CACHE_KEYS_.NUMBERS);if(cached!==null){logPerformance_('getNumbers',startedAt,{cache:'hit'});return cached;}
